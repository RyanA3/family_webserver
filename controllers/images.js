...
const errMsg = pug.compileFile(
  `${root_directory}/views/components/error_message.pug`
);
...
router.post("/upload", (req, res) => {
  multi_upload(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      res
        .send(errMsg({ name: err.name, stack: err.stack }))
        .status(500)
        .end();
      return;
    } else if (err) {
...
var image_cards = pug.compileFile(
  `${root_directory}/views/components/image_cards.pug`
);
router.get("/images/:page", async (req, res) => {
  ...
  if (
    (req.query &&
      req.query.showDuplicates &&
      req.query.showDuplicates == "no") ||
    !req.query.showDuplicates
  ) {
    filter = {
      ...filter,
  ...
  if (
    req.query &&
    req.query.filterBefore &&
    req.query.filterBefore !== "null"
  ) {
    filter = {
      ...filter,
      created: {
  ...
  if (
    req.query &&
    req.query.filterCamera &&
    req.query.filterCamera !== "null"
  ) {
    filter = {
      ...filter,
      $or: [
  ...
  if (
    req.query &&
    req.query.filterOriginalName &&
    req.query.filterOriginalName !== "null"
  ) {
    filter = {
      ...filter,
      original_name: {
  ...
  try {
    ...
    if (metas.length == 0) {
      res
        .status(200)
        .send('<div id="imageCards"><div>Nothing Found</div></div>');
      return;
    }
    ...
    if (!is_production)
      image_cards = pug.compileFile(
        `${root_directory}/views/components/image_cards.pug`
      );
    res
      .status(200)
      .send(image_cards({ imageNames: imageNames, imageMetas: displayInfo }));
  } catch (e) {
...
/*
 * Finds the first image in the database marked as a duplicate
 * Returns its metadata in JSON
 */
router.get("/firstDuplicate", async (req, res) => {
  filter = {
    "duplicates.0": {
      $exists: true,
    },
  ...
  var firstDuplicate;
  try {
    firstDuplicate = await ImageMeta.findOne(filter);
    res.status(200).send(firstDuplicate);
  } catch (e) {
    console.error(e);
    res.status(500).send("Error Finding Image Duplicate");
  }
});

/*
 * Returns a list of image cards (HTML)
 */
router.get("/ids/", async (req, res) => {
  ids = req.query.ids;
  console.log("Getting images with ids: ", ids);
  ...
  try {
    var images = await ImageMeta.find(filter);
...
var pageChanger = pug.compileFile(
  `${root_directory}/views/components/page_changer.pug`
);
router.get("/pageChanger/:page", (req, res) => {
  if (!is_production)
    pageChanger = pug.compileFile(
      `${root_directory}/views/components/page_changer.pug`
    );
...
var fullImageCard = pug.compileFile(
  `${root_directory}/views/components/full_image_card.pug`
);
